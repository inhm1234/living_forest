...
현재 V1.73.4 focus position hotfix는 **V1.73.3에서 `내 자리` 버튼을 누를 때 내 나무 위치가 위로 튀던 문제를 고치고, 백서/시간대 수정은 유지한 버전**입니다.
...
1. 상단 버전이 `V1.73.4 focus position hotfix`로 보이는지 확인
...
- `내 자리` 버튼 클릭 시 내 나무의 부모 좌표는 고정하고, 나무 이미지/라벨만 강조되도록 수정
- 위치가 가운데 위쪽으로 튀어 보이던 transform 문제 수정
